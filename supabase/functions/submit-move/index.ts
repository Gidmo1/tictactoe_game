...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const row = Number(body.row ?? body.move_row ?? null);
    const col = Number(body.col ?? body.move_col ?? null);
    ...
    if (!Number.isInteger(row) || !Number.isInteger(col)) {
      return new Response(
        JSON.stringify({ ok: false, error: 'row and col must be integers.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }
    ...
    const { data, error } = await userClient.rpc('submit_move', {
